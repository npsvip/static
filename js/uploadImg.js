...
// 初始化html
    UploadImg.prototype.init = function () {
        ...
        var html = '<input type="file" name="multipartFile" class="' + this.options.classname + '" id="file" >'
            + '<div class="' + this.options.prevclass + '" id="previewBox" style="position: absolute; display: inline-block; z-index:1;max-width: ' + this.options.maxwidth + '; max-height: ' + this.options.maxheight + ';" ><img id="uploadImg" src="https://cdn.jsdelivr.net/gh/npsvip/static@1.0/images/uploadImg.png"  width=' + this.options.width + ' height=' + this.options.height + '>';
...
//绑定事件
    ...
    UploadImg.prototype.operationImg = function (fileObj) {
        if (fileObj.files && fileObj.files[0]) {
            ...
            var self = this;
            $.ajax({
                url: '/nps/upload', type: 'POST', data: dic, processData: false,
                contentType: false,
                dataType: 'JSON',
                headers: {
